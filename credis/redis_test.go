...
func TestRedisClient_Del(t *testing.T) {
	log.Info(redisClient.Set("aa", "aaa"))
	log.Info(redisClient.Set("bb", "bbb"))
	log.Info(redisClient.Get("aa"))
	log.Info(redisClient.Get("bb"))
	log.Info(redisClient.Del("aa", "bb"))

	log.Info(redisClient.Set("aa", "aaa"))
	log.Info(redisClient.Set("bb", "bbb"))
	log.Info(redisClient.Get("aa"))
	log.Info(redisClient.Get("bb"))
	var keys = []string{"aa", "bb"}
	log.Info(redisClient.Del(keys...))
}
...
func TestRedisClient_Zrange(t *testing.T) {
	values, err := redisClient.Zrange("zsettest", 0, 0)
	if err != nil {
		t.Fatal(err)
	}
	fmt.Println(values)
...
func TestRedisClient_ZrevRangeWithScores(t *testing.T) {
	values, err := redisClient.ZrevrangeWithscores("zsettest", 0, 0)
	if err != nil {
		t.Fatal(err)
	}
	fmt.Println(values)
...
func TestSrem(t *testing.T) {
	log.Info(redisClient.Sadd("s", "s1"))
	log.Info(redisClient.Sadd("s", "s2"))
	fmt.Println(redisClient.Srem("s", "s2"))
}
