...
func (c *RedisClient) Del(keys ...string) error {
	return c.returnError("DEL", redis.Args{}.AddFlat(keys)...)
}
...
//分值低到高排序,返回全部数据，则传参为 startIndex 0   endIndex -1
func (c *RedisClient) Zrange(key string, startIndex int64, endIndex int64) (value []string, err error) {
	return c.returnStringsError("ZRANGE", key, startIndex, endIndex)
}

//分值低到高排序,并且同步返回此分值score,返回全部数据，则传参为 startIndex 0   endIndex -1
func (c *RedisClient) ZrangeWithscores(key string, startIndex int64, endIndex int64) (value []string, err error) {
	return c.returnStringsError("ZRANGE", key, startIndex, endIndex, "withscores")
}

//分值高到低排序,返回全部数据，则传参为 startIndex 0   endIndex -1
func (c *RedisClient) Zrevrange(key string, startIndex int64, endIndex int64) (value []string, err error) {
	return c.returnStringsError("ZREVRANGE", key, startIndex, endIndex)
}

//分值高到低排序,并且同步返回此分值score,返回全部数据，则传参为 startIndex 0   endIndex -1
func (c *RedisClient) ZrevrangeWithscores(key string, startIndex int64, endIndex int64) (value []string, err error) {
...
func (c *RedisClient) returnError(commandName string, in ...interface{}) error {
	...
	_, err := conn.Do(commandName, redis.Args{}.AddFlat(in)...)
	return err
}
