...
func Byt2md5(byt []byte) string {
	return fmt.Sprintf("%x", md5.Sum(byt))
}
...
func NextId(machineId string) string {
	...
	divisor++
	return fmt.Sprintf("%d_%d%s", lastTimestamp, divisor, machineId)
}

func SocketHttp(host, message string) (body []byte, err error) {
	message = strings.ReplaceAll(message, "\r\n", "\n")
	message = strings.TrimSpace(message)
	split := strings.Split(message, "\n\n")
	if len(split) == 2 { //说明包含body,需要计算Content-Length
		ContentLength := len(split[1])                          //获取body长度
		indexStart := strings.Index(message, "Content-Length:") //获取长度行的开头下标
		if indexStart > -1 {                                    //如果包中包含长度字段，则需要替换掉长度值，否则直接把Content-Length加到头中
			indexEnd := strings.Index(message[indexStart:], "\n")                                           //获取长度行的结尾
			message = message[:indexStart+16] + fmt.Sprint(ContentLength) + message[indexStart:][indexEnd:] //替换原长度值
		} else {
			message = split[0] + "\nContent-Length: " + fmt.Sprint(ContentLength) + "\n\n" + split[1] //拼接新长度值
		}
	}
...
func CleanEmptyDir(basePath string, duration time.Duration) {
	CleanTmpFile(basePath)
	DGClear(basePath, basePath)
	for {
		time.Sleep(duration)
		CleanTmpFile(basePath)
		DGClear(basePath, basePath)
...
func CleanTmpFile(basepath string) {
	now := time.Now().Unix()
	fis, _ := ioutil.ReadDir(basepath)
	for _, v := range fis {
		name := v.Name()
		if v.IsDir() {
			CleanTmpFile(basepath + name + "/")
			continue
		}
		if strings.HasSuffix(name, ".tmp") && now-v.ModTime().Unix() > 3600 {
			os.Remove(basepath + name)
		}
	}
}

// path 根目录   localPath 当前工作目录
func DGClear(path, localPath string) {
	nowDay := time.Now().Format("20060102")
	fis, err := ioutil.ReadDir(path)
	if err != nil {
		return
	}
	if len(fis) == 0 { //最后面的文件夹没有文件了
		if path == localPath { //到了跟目录就停止,防止错删
			return
		}
		os.Remove(path)                           //删除当前文件夹
		path = strings.TrimRight(path, "/")       //去除后/符号  /aaa/ccc/ => /aaa/ccc
		lastIndex := strings.LastIndex(path, "/") //判断倒数第一个文件夹分隔符位置
		path = path[:lastIndex+1]                 //    /aaa/ccc => /aaa/
		DGClear(path, localPath)                  //查看上一级是否变成空文件夹
		return
	}

	for _, fi := range fis {
		name := fi.Name()
		if fi.IsDir() { //1级分类的文件夹，每天清理空目录
			if name == nowDay { //今天的目录不清理
				continue
			}
			DGClear(path+name+"/", localPath)
		}
	}
}
