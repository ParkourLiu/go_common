...
func colFormatField(fieldName string, value interface{}, field reflect.Value, fieldType reflect.Kind) (err error) {
	...
	case int64: //数据库中是int64
		...
		default:
			return fmt.Errorf("int64 converting %s (%q) to  %s", fieldName, value, field.Kind())
		}
	case []uint8: //数据库中是[]uint8
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			...
			if err != nil {
				return fmt.Errorf("[]uint8 converting  %s (%q) to %s: %v", fieldName, valueStr, field.Kind(), err)
			}
			field.SetInt(f64)
		case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
			f64, err := strconv.ParseUint(valueStr, 10, 64)
			if err != nil {
				return fmt.Errorf("[]uint8 converting  %s (%q) to %s: %v", fieldName, valueStr, field.Kind(), err)
			}
			field.SetUint(f64)
	...
	case time.Time: //数据库中是time.Time
		if fieldType == reflect.String {
			...
			return fmt.Errorf("time.Time converting  %s (%q) to %s", fieldName, value, field.Kind())
		}
	case string:
...
func scanSlice(v reflect.Value, tElem reflect.Type, rows *sql.Rows, colsLen int, mysqlFieldTemp, cols []string) (err error) { //对切片的扫描
	...
	case reflect.String, reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64, reflect.Float32, reflect.Float64, reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		if colsLen > 1 { //查询结果不止一个字段
			return fmt.Errorf("There are multiple fields,Please receive in []Struct or []Map")
		}
